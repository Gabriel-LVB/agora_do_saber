...
Ao concluir a fixação de um tópico, a tela oferece **Próximo tópico** quando ele não for o último da aula. Nos flashcards, o resultado mostra retenção na primeira passagem, cartões que precisaram voltar, marcações **Errei** e respostas totais, sem chamar de 100% o simples fato de todos terem sido recuperados ao final. O aluno pode rever somente os difíceis, recomeçar a sessão e selecionar os erros para Revisões. A leitura das Academias FAMED usa a variante compacta do componente compartilhado, com títulos menores e sem espaçadores artificiais entre parágrafos.
