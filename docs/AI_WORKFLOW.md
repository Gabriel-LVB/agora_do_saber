...
- Para progresso da Biblioteca, use `src/services/sharedLibraryProgress.js`; respostas devem salvar patch pequeno em `answers.{questionId}`, nao o documento inteiro.
...
- A Biblioteca compartilhada (`shared-library`) e admin-only. Use `homeCanSeeSharedLibrary` para esconder tela/nav e para impedir leituras Firestore; use `showSharedLibraryAdminTools` apenas para alternar ferramentas administrativas dentro da tela.
