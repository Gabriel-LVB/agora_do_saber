...
- O fluxo de curso começa em `VideoaulasView`: concluir uma aula só libera questões individualmente para **Revisões** quando a respectiva seleção pedagógica estiver publicada. Essenciais e erros anteriores entram; complementares/reservas ficam adormecidas até evidência de dificuldade no mesmo conceito, e desativadas não entram. Sem seleção publicada, não crie cartões novos; cartões legados ficam em `awaiting-curation`, com `dueDate: null` e fora da contagem. A introdução ativa respeita os limites de 8 erradas, 10 inéditas e 6 acertadas por dia e evita duplicatas por `cardKey`. Quando um erro libera um reforço relacionado, anexe-o uma única vez ao fim da sessão ativa da mesma modalidade; se o aluno já tiver concluído enquanto o processamento terminava, reabra a sessão diretamente nesse reforço. Não obrigue o aluno a sair e entrar novamente. A UI deve continuar como fila global, sem sessões separadas por aula; `vq_review/{aulaId}` é somente a partição física. Depois da primeira resposta real, `services/fsrsScheduler.js` usa `ts-fsrs@5.4.1`, grava `fsrs` e aplica `nextDue`; preserve `legacyFallback`, `parkedDueDate`, import dinâmico, `schedulerVersion`, `reps` e `lapses`. A reconciliação incremental dessa fila no carregamento deve ser silenciosa: não mostre toast de “plano alinhado”, migração, adoção do FSRS ou outra rotina interna ao entrar no site.
...
- Em Questões do Curso, o banco completo é a fixação inicial opcional; Revisões é o recorte longitudinal. Não obrigue o aluno a transformar todas as questões em pendência: essenciais e erros retornam, complementares/reservas acertadas permanecem fora até necessidade adaptativa.
