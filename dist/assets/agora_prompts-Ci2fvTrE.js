...
---`},n=(e=[`direct`])=>((!e||e.length===0)&&(e=[`direct`]),e.length===1?t[e[0]]||``:`MISTURE os seguintes tipos de questão ao longo das questões:\n${e.map(e=>`- ${r[e]||e}`).join(`
`)}\n\n${e.map(e=>t[e]).filter(Boolean).join(`

`)}`),r={direct:`Direta (múltipla escolha)`,vof:`Verdadeiro ou Falso`,cespe:`Certo ou Errado (CESPE)`,open:`Resposta Curta (aberta)`,essay:`Dissertativa`,old_exam:`Provas antigas`,flashcard:`Flashcards`,cloze:`Flashcards Cloze`},i=e=>e===`flashcard`||e===`cloze`,a=(e=[])=>e.length===1&&i(e[0]),o=(e=[])=>e.length===1&&e[0]===`cloze`,s=(e=[])=>o(e)?`clozes`:`flashcards`,c=(e={})=>Math.max(3,Math.min(5,Number(e.vofStatementCount)||5)),l=(e={})=>(e.questionTypes||[]).includes(`vof`)?5:Number(e.numAlternatives||5),u=(e={})=>(e.questionTypes||[]).includes(`vof`)?`\nREGRA ESPECÍFICA PARA VERDADEIRO OU FALSO:\n- Cada questão deve ter EXATAMENTE ${c(e)} afirmações.\n- Use SEMPRE 5 alternativas (A-E), cada uma com uma combinação plausível de V/F.\n- O gabarito deve ser uma dessas 5 alternativas e corresponder exatamente à sequência correta das afirmações.`:``,d=(e=[])=>o(e)?`
FORMATO OBRIGATÓRIO:
## Cloze 1
Texto: [frase curta com {{c1::termo oculto}}]
...
Antes de finalizar, revise a bateria: a quantidade de casos é adequada? cada caso aprofunda de verdade? cada questão exige uma decisão distinta? o conjunto cobre o conteúdo sem repetição?`},p=`
REGRAS DO ENUNCIADO:
...
- Tamanho ideal: suficiente para contextualizar sem ser prolixo`,m=`
...
Antes de finalizar cada questão, faça este teste mental: "dá para acertar por eliminação grosseira, tamanho da alternativa, categoria absurda ou bom senso leigo?". Se sim, refaça alternativas e/ou enunciado.`,h=`
REGRAS DA EXPLICAÇÃO:
...
Refira-se SEMPRE pelo conteúdo: "a opção que menciona X", "confundir Y com Z é um erro comum pois..."`,g=`
...
[[ALT:E]]
[por que a alternativa E está errada, se existir]`,_=(e={})=>e.adminQuestionExplanations?g:h,v={essential:`
...
- Contextualize melhor o raciocínio, conectando mecanismo, clínica, critérios e exemplos quando isso ajudar o aprendizado.`},y={essential:`
OBJETIVO DE LEITURA:
...
Os títulos com ## existem só para o sistema separar as seções; dentro de cada seção, crie um título curto em negrito para orientar a leitura quando o ## estiver oculto.`},b=`
...
O texto final precisa continuar perfeitamente compreensível quando todos esses marcadores forem removidos e os parágrafos forem exibidos em sequência.`,x={essential:`
PROFUNDIDADE DA AULA: Nível 1
...
- Contextualize mecanismo, clínica, critérios e exemplos como partes de um único raciocínio progressivo.`},S=(e,t=!1,n=!1)=>`
...
Resultado esperado: 5 alternativas com comprimento quase idêntico, onde só quem domina o conteúdo consegue identificar a correta.`,C=[`Aderência ao material base e à intenção explícita do usuário`,`Alta utilidade: cada item precisa ser testável em prova ou útil na vida real, não apenas correto`,`Cobertura suficiente dos tópicos/subtópicos sem lacunas importantes`,`Sem redundância conceitual entre questões ou flashcards`,`Enunciado sem pistas semânticas, gramaticais, de tamanho ou de categoria`,`Alternativa correta tecnicamente verdadeira, atual e sem ambiguidade`,`Distratores plausíveis, da mesma categoria semântica e com nível de especificidade semelhante`,`Distratores derivados da correta por uma corrupção conceitual decisiva, preservando algum núcleo verdadeiro sem criar uma segunda resposta correta`,`Alternativas no menor texto suficiente para responder ao alvo, sem resumos explicativos que deem pistas`,`Explicação ensina o porquê e o como do conceito, sem parafrasear a resposta`,`Explicação sem referência fixa a letras, pois o site embaralha alternativas`,`Coerência do formato obrigatório para o parser do site`],w=({subjectTitle:e=``,topicTitle:t=``,subtopics:r=[],sourceMaterials:i=``,generatedText:o=``,settings:s={}})=>{let c=s.questionTypes||[`direct`],f=a(c),h=c.every(e=>[`open`,`essay`].includes(e)),g=!f&&s.questionStyle===`mixed`,v=l(s)===4?`A) [alternativa]
B) [alternativa]
C) [alternativa]
...
C) [alternativa]
D) [alternativa]
E) [alternativa]`,y=n(c),b=!!s.adminQuestionExplanations,x=f?`
FORMATO DE SAÍDA OBRIGATÓRIO:
${d(c).trim().replace(`## Flashcard 1`,`## Flashcard N`).replace(`## Cloze 1`,`## Cloze N`)}`:h?`
FORMATO DE SAÍDA OBRIGATÓRIO:
## Questão N
${g?`Caso-base: Caso 1 — [vinheta clínica original completa]
Enunciado: [pergunta específica desta questão e eventual evolução nova]`:`[Enunciado]`}
...
${r?.length?`- Subtópicos obrigatórios:\n${r.map((e,t)=>`${t+1}. ${e}`).join(`
`)}`:``}

MATERIAL BASE / INTENÇÃO DO USUÁRIO:
${i?i.substring(0,14e3):`Não informado. Use apenas o contexto das questões e subtópicos.`}
...
Agora devolva APENAS a bateria final reparada, no formato obrigatório.`},T=(e,t=``,r=!1)=>{let i=l(e)===4?`A) [alternativa]
B) [alternativa]
C) [alternativa]
D) [alternativa]`:`A) [alternativa]
B) [alternativa]
C) [alternativa]
D) [alternativa]
E) [alternativa]`,c=f[e.questionStyle||`mixed`],h=Math.max(1,Number(e.numSubtopics)||1),g=Math.max(1,Number(e.qPerSub)||1),v=h*g,y=e.questionTypes||[`direct`],b=a(y),x=!!e.qPerSubAuto&&!b,C=!b&&e.questionStyle===`mixed`,w=b?`
ESTRUTURA PARA ${o(y)?`CLOZES`:`FLASHCARDS`}:
- Cubra os subtópicos/conceitos obrigatórios quando eles forem fornecidos.
- Não use meta numérica fixa. Crie apenas ${s(y)} de alto rendimento, sem redundância.
...
- O conjunto final deve permitir revisar o conteúdo essencial sem reler o material. Não omita mecanismo, conduta, diagnóstico, diferencial ou pegadinha central quando forem parte do tópico.`:x?`
ESTRUTURA (quantidade de questões automática):
Quando uma lista de subtópicos obrigatórios for fornecida, cubra TODOS eles.
Se NÃO houver lista obrigatória, defina mentalmente ${h} eixos/subtópicos de cobrança para este tópico.
...
- Só pare depois de verificar que os eixos importantes foram cobrados sem lacunas e sem redundância.`:r?`
...
- Gere EXATAMENTE ${v} questões no total
- Distribua como ${g} questão(ões) por eixo/subtópico
- NÃO gere apenas uma questão e NÃO pare antes de completar ${v}
Critérios:
...
- EXATAMENTE ${h} subtópicos
- EXATAMENTE ${g} questão por subtópico
- Total: EXATAMENTE ${v} questões
- Ordem: do conceito mais fundamental ao mais específico`,T=n(e.questionTypes||[`direct`]),E=y.every(e=>[`open`,`essay`].includes(e)),D=_(e),O=C?`
...
- Preserve a quantidade total pedida e cubra todos os eixos obrigatórios sem divisão artificial.`:``,k=b?`
FORMATO OBRIGATÓRIO para cada ${o(y)?`cloze`:`flashcard`} (separe com ---):
${d(y).trim()}

REGRA DE QUANTIDADE:
- Ignore qualquer quantidade fixa citada em outras seções.
- Gere a quantidade ideal de ${s(y)} para cobrir os conceitos essenciais do tópico e dos subtópicos, sem redundância.
...
## Questão 1.1.1
${C?`Caso-base: Caso 1 — [vinheta clínica original completa; repita-a integralmente em cada questão da sequência]
Enunciado: [pergunta específica desta questão e eventual evolução nova]`:`[Enunciado]`}
Resposta esperada: [resposta objetiva]
Explicação: [explicação didática]
---`:`${m}\n${D}\n${S(i,!!e.adminQuestionExplanations,C)}`;return`Você é o Oráculo de Medicina da Ágora do Saber. Sua missão é criar questões médicas de altíssima qualidade para residência médica.

${t?t+`
`:``}
ESTILO DE ENUNCIADO: ${c}
${T?T+`
`:``}${u(e)}
${w}
${O}
...
${b?`Use o ID no formato sequencial simples (ex: ## ${o(y)?`Cloze`:`Flashcard`} 1, ## ${o(y)?`Cloze`:`Flashcard`} 2).`:`Use o ID no formato TOPICO.SUBTOPICO.QUESTAO, sem colchetes (ex: ## Questão 3.2.1).`}
${b?`Gere os ${s(y)} sem interromper. Não resuma, não pergunte, não comente — apenas ${s(y)}.`:`Gere TODAS as questões sem interromper. Não resuma, não pergunte, não comente — apenas questões.`}`},E=`PRINCÍPIO DE EFICIÊNCIA E ALTO RENDIMENTO:
...
O objetivo é maximizar domínio relevante por tempo de estudo: cobertura essencial sem superficialidade, redundância ou expansão por completude aparente.`,D=(e,t,n=!1)=>`Você é o Arquiteto de Alexandria. Crie um sumário para "${e}" baseado no material do usuário.
...
Responda APENAS o sumário.`,O=(e,t,n)=>`Você é o Arquiteto de Alexandria. Ajuste o sumário abaixo conforme a instrução do usuário.
...
- Responda APENAS o sumário revisado, sem comentários adicionais`,k=e=>{let t=l(e),i=Math.max(1,Number(e.qPerSub)||1),o=e.questionTypes?.[0]||`direct`,d=[o];if(o===`old_exam`)return`[INSTRUÇÕES PARA TRANSCREVER PROVAS ANTIGAS — ÁGORA DO SABER]
...
- Aguarde eu enviar a prova antes de responder.`;let h=d.some(e=>[`direct`,`vof`,`cespe`].includes(e)),g=a(d),v=n(d),y=_(e),b=t===4?`A) [alternativa]
B) [alternativa]
C) [alternativa]
D) [alternativa]`:`A) [alternativa]
B) [alternativa]
...
Responda APENAS o sumário. Aguarde a confirmação antes de gerar questões.`,C=e.autoMode?`*** PARTE 2: GERAÇÃO (um tópico por vez) ***
Para cada tópico gere ${g?`a quantidade ideal de ${s(d)}, sem meta fixa`:`${i} item(ns) por subtópico (total = subtópicos daquele tópico × ${i})`}.`:`*** PARTE 2: GERAÇÃO (um tópico por vez) ***
Para cada tópico gere ${g?`a quantidade ideal de ${s(d)}, sem meta fixa`:`${e.numSubtopics*i} item(ns) (${e.numSubtopics} subtópicos × ${i} por subtópico)`}.`,w=h?`
FORMATO PARA QUESTÕES COM ALTERNATIVAS:
${S(b,!!e.adminQuestionExplanations,e.questionStyle===`mixed`)}`:``,T=o===`direct`?`Use exclusivamente blocos "## Questão N" com ${t} alternativas, "Alternativa correta: [letra]" e "Explicação:".`:o===`vof`?`Use exclusivamente blocos "## Questão N" com exatamente ${c(e)} assertivas em numerais romanos, 5 alternativas A-E com combinações V/F, "Alternativa correta: [letra]" e "Explicação:".`:o===`cespe`?`Use exclusivamente blocos "## Questão N" com A) Certo, B) Errado, "Alternativa correta: [letra]" e "Explicação:".`:o===`open`?`Use exclusivamente blocos "## Questão N" com "Resposta esperada:" e "Explicação:", sem alternativas.`:o===`essay`?`Use exclusivamente blocos "## Questão N", "Tipo: Dissertativa", "Resposta esperada:" e "Explicação:", sem alternativas.`:o===`flashcard`?`Use exclusivamente blocos "## Flashcard N" com "Pergunta:", "Resposta:" e "Explicação:".`:`Use exclusivamente blocos "## Cloze N" com "Texto:" contendo {{c1::...}} e "Extra:".`;return`[INSTRUÇÕES PARA IA EXTERNA — ÁGORA DO SABER]
...
${v?`${v}\n`:``}
${u(e)}
ESTILO: ${f[e.questionStyle||`mixed`]}
${p}
${h?m:``}
${h?y:``}
${w}
...
- Não coloque comentários fora dos blocos de itens, porque vou colar a resposta diretamente no importador do Ágora.`},A=(t,n,r,i,a=``,o={})=>{let s=e.videoaulas.minSubtopicsPerBlock,c=Math.max(s,Math.min(30,o.maxSubtopicsPerBlock||e.videoaulas.maxSubtopicsPerBlock)),l=Math.max(s,Math.min(c,r||6)),u=!!o.fullCoverage,d=o.openStructure?`ESTRUTURA ABERTA E COMPLETA:
...
- Nunca coloque um detalhe, exceção ou efeito adverso antes de ter coberto o conceito principal`;return`Você é um professor de Medicina e editor de material didático. Sua tarefa é criar o sumário completo da aula "${t.title}".
...
${i?`TRANSCRIÇÃO DA AULA (use como base para os subtópicos):\n${o.fullTranscript?i:i.substring(0,25e3)}`:`[Sem transcrição disponível — baseie-se no título: "${t.title}"]`}`},j=(e,t,r,i,c)=>{let l=f[t.questionStyle||`mixed`],h=r.length||t.qPerBlock||5,g=t.questionTypes||[`direct`],v=a(g),y=!v&&t.questionStyle===`mixed`,b=n(g),x=_(t);return`Você é um examinador de residência médica criando questões sobre "${e.title}" (aula: ${t.aulaTitle}).

ESTILO: ${l}
${b?`${b}\n`:``}
${u(t)}

SUBTÓPICOS (${v?`cubra os conceitos essenciais, sem quantidade fixa`:y?`cubra todos; os casos podem integrar vários subtópicos relacionados`:`gere 1 questão por subtópico, nesta ordem exata`}):
${r.map((e,t)=>`${t+1}. ${e}`).join(`
`)}

${v?`QUANTIDADE: a IA deve decidir a quantidade ideal de ${s(g)}, cobrindo alto rendimento sem repetição. O conjunto deve permitir revisar a aula/bloco sem reler a transcrição, com cobranças específicas e explicações que ensinem o porquê/como da resposta. Corte cartões de conselho geral e mantenha só itens testáveis em prova ou úteis na vida real.`:`TOTAL: EXATAMENTE ${h} questões.`}
${y?`ORGANIZAÇÃO: decida a quantidade ideal de casos para o bloco inteiro. Não crie um caso por subtópico; um caso pode integrar vários subtópicos. Organize a bateria por caso e cubra todos os subtópicos ao longo dela.`:``}
${p}
${v?d(g):`${m}
${x}
${S(c,!!t.adminQuestionExplanations,y)}`}

Use o ID como número sequencial simples (${v?`${o(g)?`Cloze`:`Flashcard`} 1, ${o(g)?`Cloze`:`Flashcard`} 2...`:`1, 2, 3...`}).

${i?`REFERÊNCIA DO CONTEÚDO (trecho da aula):\n${i.substring(0,4e4)}`:`[Sem transcrição — baseie-se nos subtópicos e no título da aula]`}

${v?`Gere os ${s(g)} sem interromper ou comentar.`:`Gere TODAS as ${h} questões sem interromper ou comentar.`}`},M=({lessonTitle:e=``,batchTitle:t=``,subtopics:n=[],transcript:r=``,alts:i=``,meta:a={}})=>{let o=_({...a,questionStyle:`direct`,questionTypes:[`direct`]});return`Você é um examinador de residência médica criando uma bateria de FIXAÇÃO DIRETA sobre a aula "${e}".
...
REFERÊNCIA DA AULA:
${String(r||``).substring(0,4e4)}`},N=({lessonTitle:e,focusBlock:t,allBlocks:n=[],transcript:r=``,alts:i})=>{let a=t?.subtopics||[],o=n.map((e,t)=>`TÓPICO ${t+1}: ${e.title}\n${(e.subtopics||[]).map(e=>`- ${e}`).join(`
`)}`).join(`
...
${r}`},P=(e,t,n=!1)=>`Você é o Arquiteto de Alexandria, construindo o sumário de um curso sobre "${e}".
...
Antes de responder, confira se cada seção respeita o limite do nível escolhido.`},I=(e,t,r,i=``,c=null,h=``)=>{let g=l(r)===4?`A) [alternativa]
B) [alternativa]
C) [alternativa]
D) [alternativa]`:`A) [alternativa]
B) [alternativa]
C) [alternativa]
D) [alternativa]
E) [alternativa]`,v=f[r.questionStyle||`mixed`],y=n(r.questionTypes||[`direct`]),b=r.questionTypes||[`direct`],x=a(b),C=!x&&r.questionStyle===`mixed`,w=_(r),T=Array.isArray(e)?e:[e],E=Array.isArray(c)&&c.length?c.map(e=>Math.max(1,Math.min(30,Number(e)||1))):T.map(()=>2),D=E.reduce((e,t)=>e+t,0);return`Você é um examinador de residência médica criando questões de fixação para "${t}".

ESTILO: ${v}
${y?y+`
`:``}${u(r)}
SUBTÓPICOS DA AULA${x?` (cubra alto rendimento, sem quantidade fixa)`:` E QUANTIDADE OBRIGATÓRIA`}:
${T.map((e,t)=>x?`${t+1}. ${e}`:`${t+1}. ${e} → ${E[t]||2} questões`).join(`
`)}

${x?`QUANTIDADE: gere a quantidade ideal de ${s(b)} para revisar o essencial da aula, sem redundância. O conjunto deve permitir reconstruir a aula ativa e clinicamente, como uma revisão AnKing-like.`:`TOTAL OBRIGATÓRIO: EXATAMENTE ${D} questões.`}

REGRA DE FIXAÇÃO (CRÍTICA):
- ${x?`Não use mínimo fixo por subtópico; use o menor conjunto de cartões que preserve cobertura de alto rendimento.`:C?`Use a quantidade individual indicada como meta de COBERTURA de cada subtópico, mas organize a bateria pelos casos. Um caso pode integrar vários subtópicos relacionados; não crie um caso por subtópico.`:`Siga exatamente a quantidade individual indicada acima. Quando o plano pedir 1, faça uma única questão forte e suficiente.`}
...
- Cada subtópico deve ter ${x?`cartões`:`questões`} suficientes para revisar seus conceitos centrais sem virar repetição.
...
- Antes de finalizar, confira se cada subtópico recebeu exatamente a quantidade pedida e se não há repetição conceitual.${C?` Confira também se a quantidade de casos foi decidida para o tópico inteiro e se cada sequência aprofunda o caso de verdade.`:``}
...
- Não repita o gabarito com mais palavras.`:w}
${x?d(b):S(g,!!r.adminQuestionExplanations,C)}

${x?`Use IDs sequenciais simples: ## ${o(b)?`Cloze`:`Flashcard`} 1, ## ${o(b)?`Cloze`:`Flashcard`} 2...`:`Use IDs no formato SUBTOPICO.QUESTAO, sem colchetes, apenas para indicar o subtópico MAIS RELACIONADO à questão:
## Questão 1.1
## Questão 1.2
## Questão 1.3
## Questão 2.1
${C?`A ordem dos IDs pode alternar entre subtópicos para preservar a sequência do caso.`:`Não pule subtópicos.`} Não crie IDs fora do plano.`}

${i?`CONTEXTO DA AULA:\n${i.substring(0,12e3)}`:``}

${h?`QUESTÕES JÁ EXISTENTES SOBRE ESTA AULA (não copie; varie foco, cenário e distratores):\n${h.substring(0,8e3)}`:``}

${x?`Gere a bateria de ${s(b)} sem interromper.`:`Gere a bateria de fixação completa sem interromper.`}`},L=(e,t,r,i=``,c=``,h=null)=>{let g=l(r)===4?`A) [alternativa]
B) [alternativa]
C) [alternativa]
D) [alternativa]`:`A) [alternativa]
B) [alternativa]
C) [alternativa]
D) [alternativa]
E) [alternativa]`,v=f[r.questionStyle||`mixed`],y=n(r.questionTypes||[`direct`]),b=r.questionTypes||[`direct`],x=a(b),C=!x&&r.questionStyle===`mixed`,w=_(r),T=Array.isArray(t)?t:[t],E=Array.isArray(h)&&h.length?h.map(e=>Math.max(1,Math.min(30,Number(e)||1))):T.map(()=>2),D=E.reduce((e,t)=>e+t,0);return`Você é o Oráculo de Medicina da Ágora do Saber, gerando uma bateria de revisão sobre "${e}".

ESTILO: ${v}
${y?y+`
`:``}${u(r)}
ESTRUTURA${x?``:` E QUANTIDADE OBRIGATÓRIA`}:
${T.map((e,t)=>x?`- Subtópico ${t+1}: "${e}"`:`- Subtópico ${t+1}: "${e}" → ${E[t]||2} questões`).join(`
`)}
${x?`Quantidade: gere a quantidade ideal de ${s(b)}, cobrindo alto rendimento sem repetição. O conjunto deve permitir revisar ativamente o essencial sem reler a aula.`:`Total: EXATAMENTE ${D} questões, na ordem acima.`}

REGRA DA BATERIA EXTRA:
- ${x?`Use a mesma lógica dos flashcards de fixação: atomização, alto rendimento, zero ambiguidade, recuperação ativa e menor número útil de cartões.`:C?`Use as quantidades por subtópico como metas de cobertura, mas decida a quantidade ideal de casos para o tópico inteiro. Um caso pode integrar vários subtópicos relacionados; não crie um caso por subtópico.`:`Siga exatamente a quantidade indicada para cada subtópico, inclusive quando for 1.`}
...
- Cubra todos os subtópicos e não crie questões fora do plano.${C?` Organize a ordem final pelas sequências dos casos, não pela lista de subtópicos.`:``}

${p}
${x?``:`${m}
${w}
${S(g,!!r.adminQuestionExplanations,C)}`}
${x?d(b):``}

${x?`Use IDs sequenciais simples: ## ${o(b)?`Cloze`:`Flashcard`} 1, ## ${o(b)?`Cloze`:`Flashcard`} 2...`:`Use o ID no formato SUBTOPICO.QUESTAO, sem colchetes (ex: ## Questão 1.1, ## Questão 1.2, ## Questão 2.1...).`}
...
${x?`Gere os ${s(b)} sem interromper.`:`Gere TODAS as questões sem interromper.`}`};export{C as QUESTION_REPAIR_CHECKLIST,f as STYLE_INST,e as SYLLABUS_LIMITS,t as TYPE_INST,L as buildAcademiaExtraBatteryPrompt,I as buildAcademiaFixationPrompt,F as buildAcademiaLessonPrompt,P as buildAcademiaSyllabusPrompt,k as buildExternalPrompt,T as buildOracleQuestionPrompt,D as buildOracleSyllabusPrompt,O as buildOracleSyllabusRevisePrompt,w as buildQuestionRepairPrompt,N as buildSharedLibraryClinicalPrompt,M as buildSharedLibraryDirectPrompt,n as buildTypeInst,j as buildVqBlockPrompt,A as buildVqSyllabusPrompt};