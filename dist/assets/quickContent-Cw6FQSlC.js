var e=(e=``)=>String(e||``).replace(/\*/g,``),t=(e=``,t=``)=>{let n=String(e||``).replace(/\r\n/g,`
`),r=t.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),i=RegExp(`(?:^|\\n)#{2,4}\\s*${r}\\s*\\n([\\s\\S]*?)(?=(?:^|\\n)#{2,4}\\s*(?:Aula\\s+r[aá]pida|Explica[çc][aã]o|Quest[õo]es|Flashcards)\\b|$)`,`i`);return(n.match(i)?.[1]||``).trim()},n=(n=``)=>{let r=String(n||``).replace(/\r\n/g,`
`);return e(((r.match(/(?:^|\n)##\s*Aula\s+r[aá]pida\s*\n([\s\S]*?)(?=(?:^|\n)##\s*(?:Quest[õo]es|Flashcards)\b|$)/i)||r.match(/(?:^|\n)##\s*Explica[çc][aã]o\s*\n([\s\S]*?)(?=(?:^|\n)##\s*(?:Quest[õo]es|Flashcards)\b|$)/i))?.[1]||t(r,`Aula rápida`)||``).trim().replace(/:\*/g,`:`)).replace(/\n{3,}/g,`

`).trim()},r=(t=``,n=``)=>e((String(t||``).replace(/\r\n/g,`
`).match(/(?:^|\n)##\s*T[íi]tulo\s*\n([\s\S]*?)(?=(?:^|\n)##\s*(?:Foco|Aula\s+r[aá]pida|Explica[çc][aã]o)\b|$)/i)?.[1]||n||``).trim().split(`
`)[0]||``).replace(/^["'“”]+|["'“”]+$/g,``).trim(),i=(t=``,n=``)=>e((String(t||``).replace(/\r\n/g,`
`).match(/(?:^|\n)##\s*Foco\s*\n([\s\S]*?)(?=(?:^|\n)##\s*(?:Aula\s+r[aá]pida|Explica[çc][aã]o)\b|$)/i)?.[1]||n||``).trim()).replace(/\n{3,}/g,`

`).trim(),a=({context:e=``,explanationLength:t=`essential`})=>{let n={essential:`PROFUNDIDADE: Nível 1 — resposta curta e ultra-direta.
...
`.trim()},o=({title:e=``,context:t=``,lesson:n=``,intent:r=``,settings:i={},outputs:a=[`questions`,`flashcards`],alternativeCount:o=5,vofRule:s=``,distractorRule:c=``})=>{let l=Array.isArray(a)?a.filter(e=>[`questions`,`flashcards`].includes(e)):[`questions`,`flashcards`],u=l.includes(`questions`),d=l.includes(`flashcards`),f=[u?`questões`:null,d?`flashcards`:null].filter(Boolean).join(` e `),p=o===4?`A) [alternativa]
B) [alternativa]
C) [alternativa]
...
CONTEXTO DO USUÁRIO:
${t||`Sem contexto adicional. Explique o tema do zero, mas sem fugir do foco.`}

AULA JÁ GERADA:
${n||`Sem aula enviada. Use o contexto e o foco acima.`}

OBJETIVO:
- Gere somente: ${f||`nenhum conteúdo ativo`}.
- ${u?`Crie uma quantidade compacta de questões de fixação que testem se o usuário realmente entendeu essa lacuna.`:`Não gere questões nem a seção "Questões".`}
- ${d?`Crie flashcards atômicos e exportáveis ao Anki para impedir que a lacuna volte.`:`Não gere flashcards nem a seção "Flashcards".`}
...
- Questões com alternativas devem ter exatamente ${o} alternativas no formato:
${p}
${s}
${c}
...
${p}
Gabarito: A
Explicação:${i.adminQuestionExplanations?`
Aula:
...
---
`.trim()};export{a as buildQuickLessonPrompt,o as buildQuickPracticePrompt,i as extractQuickIntent,n as extractQuickLesson,t as extractQuickSection,r as extractQuickTitle};