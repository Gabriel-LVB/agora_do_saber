...
const Settings2 = ({ className }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M20 7h-9"/><path d="M14 17H5"/><circle cx="17" cy="17" r="3"/><circle cx="7" cy="7" r="3"/></svg>);
...
const formatText = (text, darkMode) => {
  ...
  return parts.map((part, index) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      ...
      return <strong key={index} className={`font-bold ${textColor}`}>{part.slice(2, -2)}</strong>;
    }
  ...
  return (
    <div className={`${cardBg} rounded-xl shadow-sm border p-4 md:p-6 mb-6 transition-all`}>
      <div className="flex items-center justify-between mb-4">
        <span className={`${tagBg} text-xs md:text-sm font-bold px-3 py-1 rounded-full uppercase tracking-wider`}>
          Prova {question.id !== 'N/A' ? question.id : index + 1}
        </span>
      </div>

      <div className={`${statementColor} text-base md:text-lg mb-6 leading-relaxed whitespace-pre-wrap`}>
      ...
      <div className="space-y-3 mb-6">
        {question.options.map((opt) => {
          ...
          let badgeClass = "flex-shrink-0 w-8 h-8 rounded flex items-center justify-center font-bold mr-3 md:mr-4 transition-colors ";
          ...
          if (!isAnswered) {
            ...
            badgeClass += darkMode ? "bg-gray-700 text-gray-400" : "bg-gray-100 text-gray-500";
          } else {
            ...
            if (opt.isCorrect) {
              ...
              badgeClass += darkMode ? "bg-gray-800 text-gray-600" : "bg-gray-100 text-gray-400";
            }
          }
      ...
      {isAnswered && question.explanation && (
        <div className={`mt-6 p-4 md:p-5 ${expBoxBg} rounded-xl border leading-relaxed whitespace-pre-wrap animate-in fade-in slide-in-from-top-4 duration-300 text-sm md:text-base`}>
          <h4 className={`font-bold ${expTitleColor} mb-3 flex items-center gap-2 uppercase tracking-wide text-sm`}>
...
export default function QuestionBankApp() {
  ...
  useEffect(() => { localStorage.setItem('qb_view', view); }, [view]);
  useEffect(() => { localStorage.setItem('qb_rawText', rawText); }, [rawText]);
  ...
  const allAnswered = parsedData.questions.length > 0 && Object.keys(userAnswers).length === parsedData.questions.length;
  ...
  const statsBadge = darkMode ? "bg-gray-700 text-yellow-400 text-opacity-80" : "bg-gray-100 text-yellow-700 text-opacity-80";
  const themeBtn = darkMode ? "text-yellow-400 hover:bg-gray-700" : "text-yellow-600 hover:bg-gray-100";
      ...
      <header className={`${headerBg} border-b sticky top-0 z-10 shadow-sm transition-colors duration-300`}>
        <div className="max-w-5xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            {view === 'bank' && (
              <div className={`hidden md:block text-sm font-semibold px-3 py-1 rounded-full ${statsBadge}`}>
      ...
      <main className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-8 relative">
        {view === 'input' && (
          <div className="animate-in fade-in duration-500">
            <div className="mb-6">
              <h2 className={`text-xl md:text-3xl font-serif font-bold mb-2 ${titleColor}`}>Oráculo de Questões</h2>
              <p className={`text-sm md:text-base ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                Deposite aqui os pergaminhos gerados pela IA. O Oráculo irá decifrá-los, separar os ensinamentos e <strong>embaralhar as provações</strong>.
              </p>
            </div>

            <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl shadow-sm border overflow-hidden flex flex-col h-[50vh] md:h-[60vh] transition-colors duration-300`}>
              <div className={`${darkMode ? 'bg-gray-800 bg-opacity-80 border-gray-700 text-gray-400' : 'bg-gray-50 border-gray-200 text-gray-600'} border-b p-3 flex items-center gap-2 text-xs md:text-sm font-medium`}>
          ...
          <div className="animate-in slide-in-from-bottom-8 duration-500">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between mb-6 md:mb-8 gap-3 md:gap-4">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <div className={`hidden sm:flex text-xs md:text-sm items-center gap-2 px-3 py-2 border rounded-lg ${darkMode ? 'bg-gray-800 border-gray-700 text-gray-400' : 'bg-white border-gray-200 text-gray-500'}`}>
            ...
            {parsedData.questions.length === 0 ? (
              <div className={`text-center py-16 md:py-20 rounded-xl border border-dashed ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-300'}`}>
                <Landmark className={`w-10 h-10 md:w-12 md:h-12 mx-auto mb-4 ${darkMode ? 'text-gray-600' : 'text-gray-300'}`} />
                <h3 className={`text-base md:text-lg font-serif font-bold ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Nenhum Conhecimento Decifrado</h3>
                <p className={`text-sm md:text-base px-4 mt-2 ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Verifique se os pergaminhos colados seguem o formato correto.</p>
              </div>
            ) : (
              <div className="space-y-4 md:space-y-6">
                ...
                {allAnswered && parsedData.summary && !showSummary && (
                  <div className={`mt-8 md:mt-12 rounded-2xl shadow-xl border p-6 md:p-10 relative overflow-hidden text-center animate-in fade-in zoom-in duration-500 ${darkMode ? 'bg-gray-800 border-yellow-600' : 'bg-white border-yellow-400'}`}>
                    <Award className={`w-16 h-16 md:w-20 md:h-20 mx-auto mb-4 ${darkMode ? 'text-yellow-500' : 'text-yellow-500'}`} />
                    <h3 className={`text-2xl md:text-3xl font-serif font-bold mb-3 ${darkMode ? 'text-yellow-400' : 'text-yellow-700'}`}>
                      As Provações Foram Vencidas!
                    </h3>
                    <p className={`text-base md:text-lg mb-8 max-w-2xl mx-auto ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                ...
                {showSummary && parsedData.summary && (
                  <div className={`mt-8 md:mt-12 rounded-2xl shadow-xl border p-6 md:p-8 relative overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700 ${darkMode ? 'bg-gray-800 border-yellow-600' : 'bg-yellow-100 border-yellow-400'}`}>
                    <div className="absolute top-0 right-0 p-4 md:p-8 opacity-10 pointer-events-none">
                      <Flame className={`w-24 h-24 md:w-32 md:h-32 ${darkMode ? 'text-white' : 'text-yellow-800'}`} />
                    </div>
                    <div className="relative z-10">
                      <div className={`flex items-center gap-2 md:gap-3 mb-4 ${darkMode ? 'text-yellow-400' : 'text-yellow-700'}`}>
                        <Flame className="w-5 h-5 md:w-6 md:h-6" />
                        <h3 className="text-lg md:text-xl font-serif font-bold tracking-wide uppercase">Consolidação do Saberes</h3>
                      </div>
                      <div className={`text-base md:text-lg leading-relaxed whitespace-pre-wrap font-medium ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>
          ...
          <div className={`w-full max-w-md rounded-2xl shadow-2xl border p-6 md:p-8 animate-in zoom-in-95 duration-200 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className={`text-xl md:text-2xl font-serif font-bold mb-2 ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                Purificar Pergaminhos?
              </h3>
              <p className={`mb-8 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                Deseja apagar todas as marcações e enfrentar as provações novamente? Sua sabedoria atual será esquecida no tempo.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 w-full">
                ...
                <button
                  ...
                  className="flex-1 py-3 px-4 font-medium rounded-xl bg-red-600 hover:bg-red-700 text-white shadow-md transition-all flex justify-center items-center gap-2"
                >
                  <RotateCcw className="w-4 h-4" />
                  Apagar Tudo
                </button>
