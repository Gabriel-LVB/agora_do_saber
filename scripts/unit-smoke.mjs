...
const dailyMigrationQuestions = Array.from({ length:45 }, (_, index) => ({
  ...
  learningPolicy:{
    tier:index < 15 ? 'essential' : index < 35 ? 'complementary' : 'reserve',
    conceptIds:[`daily-concept-${index}`],
    primaryConceptId:`daily-concept-${index}`,
    importance:index < 15 ? 5 : index < 35 ? 4 : 2,
    qualityScore:100 - index,
    learningRole:index < 15 ? 'core' : index < 35 ? 'reinforcement' : 'variation',
    reviewEligible:true,
    status:'active',
  },
}));
const dailyInitialPlan = buildWatchedLessonsIndividualPlan({
  now:reviewNow,
  lessons:[{ aulaId:'daily-migration', aulaData:{ blocks:{ main:{ questions:dailyMigrationQuestions } } } }],
});
const dailyV9Queue = JSON.parse(JSON.stringify(dailyInitialPlan.queue));
Object.values(dailyV9Queue['daily-migration'].main).forEach(item => {
  item.dueDate = reviewNow;
  item.migration.version = 'curated-progressive-essential-fsrs-v9';
  item.migration.firstExposure.version = 'curated-progressive-essential-fsrs-v9';
});
const reviewedDailyIds = ['daily-00','daily-01','daily-02'];
reviewedDailyIds.forEach((qId, index) => {
  const item = dailyV9Queue['daily-migration'].main[qId];
  item.reps = index + 1;
  item.lastReview = reviewNow + index;
  item.dueDate = reviewNow + (60 + index) * REVIEW_DAY_MS;
});
const dailyMigrationAt = reviewNow + 2 * REVIEW_DAY_MS;
const dailyMigratedPlan = buildWatchedLessonsIndividualPlan({
  now:dailyMigrationAt,
  existingQueue:dailyV9Queue,
  lessons:[{ aulaId:'daily-migration', aulaData:{ blocks:{ main:{ questions:dailyMigrationQuestions } } } }],
});
reviewedDailyIds.forEach((qId, index) => {
  const item = dailyMigratedPlan.queue['daily-migration'].main[qId];
  assert.equal(item.dueDate, reviewNow + (60 + index) * REVIEW_DAY_MS);
  assert.equal(item.reps, index + 1);
  assert.equal(item.migration.firstExposure.version, 'curated-progressive-essential-fsrs-v9');
});
const unseenDailyItems = Object.entries(dailyMigratedPlan.queue['daily-migration'].main)
  .filter(([qId]) => !reviewedDailyIds.includes(qId))
  .map(([, item]) => item);
...
const redistributedLegacyV5Plan = buildWatchedLessonsIndividualPlan({
  now:reviewNow + 2000,
  existingQueue:legacyV5Queue,
  lessons:[siblingAwareLesson],
});
assert.notDeepEqual(
  Object.fromEntries(Object.entries(redistributedLegacyV5Plan.queue['sibling-aware'].main)
...
assert.match(appSource, /contentDocIds:needsSharedLibraryUiData \? \[\] : courseSharedLibraryDocIds/);
...
assert.match(appSource, /const isConstrainedConnection =/);
assert.match(appSource, /connection\?\.saveData/);
assert.match(appSource, /requestIdleCallback/);
assert.match(appSource, /scheduleStage\(1, 1500\)/);
assert.match(appSource, /scheduleStage\(2, 6000\)/);
...
assert.match(appSource, /Não foi possível confirmar seu acesso/);
assert.match(appSource, /setAuthRetryKey\(value => value \+ 1\)/);
assert.match(appSource, /const retryVideoaulas = \(\) =>/);
assert.match(appSource, /videoaulasLoadedRef\.current = false;/);
assert.match(appSource, /setVideoaulasLoadError/);
...
assert.match(appSource, /deleteImportedTopic/);
assert.match(appSource, /type:'external-topic'/);
assert.match(appSource, /deleteLibraryTopicProgress/);
...
assert.match(libraryProgressSource, /deleteLibraryTopicProgress/);
assert.match(libraryProgressSource, /deleteDoc/);
...
assert.match(sharedLibrarySyncSource, /contentDocIds = \[\]/);
...
assert.match(sharedLibrarySyncSource, /requestedDocIds\.map\(id => withFirestoreTimeout\(getDoc/);
assert.match(sharedLibrarySyncSource, /hydrateSharedLibraryItemIds/);
assert.match(sharedLibrarySyncSource, /hydrateSharedLibraryItems\(targets, 3\)/);
...
assert.match(appSource, /\.flashcard-actions-popover,[\s\S]*?max-height: calc\(100dvh - 5\.5rem\)/);
const ankiPackageSource = await readFile(new URL('../src/features/exporting/ankiPackage.js', import.meta.url), 'utf8');
assert.match(ankiPackageSource, /'collection\.anki2'/);
assert.match(ankiPackageSource, /CREATE TABLE notes/);
assert.match(ankiPackageSource, /CREATE TABLE cards/);
assert.match(ankiPackageSource, /sql\.js\/dist\/sql-wasm\.wasm\?url/);
assert.match(appSource, /const ANKI_AGORA_DECK = 'Ágora do Saber'/);
assert.match(appSource, /import\('\.\/features\/exporting\/ankiPackage\.js'\)/);
assert.match(appSource, /tagLevels\.slice\(0, levelIndex \+ 1\)\.join\('::'\)/);
...
const compactSiteIcon = await readFile(new URL('../public/brand/agora-icon-32.png', import.meta.url));
const compactTouchIcon = await readFile(new URL('../public/brand/agora-icon-180.png', import.meta.url));
const compactManifestIcon = await readFile(new URL('../public/brand/agora-icon-192.png', import.meta.url));
assert.ok(compactSiteIcon.length < 10_000);
assert.ok(compactTouchIcon.length < 100_000);
assert.ok(compactManifestIcon.length < 100_000);
